-- Generator srpskog jezika koristi i posebnu oblast „Vrste reči“. U postojećim
-- projektima ona možda već postoji iz CSV uvoza, pa migracija ne menja taj red.
insert into public.topics (slug, name, sort_order, subject, grade) values
  ('srpski-vrste-reci', 'Vrste reči', 190, 'srpski', 3)
on conflict (slug) do nothing;

-- Dnevni generator sada podržava srpski jezik, pa ograničenje na matematiku
-- više nije deo modela. Izbor stvarno podržanih oblasti kontroliše aplikacija.
alter table public.daily_quiz_schedules
  drop constraint daily_quiz_schedules_check;
