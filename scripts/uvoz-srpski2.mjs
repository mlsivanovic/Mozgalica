...
import { SRPSKI2_BANKA } from '../src/data/srpski2Banka.ts'

export function napraviSqlUvoza(primeni = false) {
  const payload = JSON.stringify(SRPSKI2_BANKA).replaceAll("'", "''")
  return `begin;
create temporary table srpski2_uvoz on commit drop as
...
begin
  ...
  if exists (
    select 1 from srpski2_uvoz u left join public.topics t on t.slug = u."topicSlug"
    where t.id is null or t.subject <> 'srpski' or t.grade <> 2
  ) then
    raise exception 'Nedostaju oblasti srpskog za 2. razred. Najpre primeniti migraciju.';
  end if;
  ...
  from srpski2_uvoz u join public.topics t on t.slug = u."topicSlug"
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!['--dry-run', '--apply', '--sql'].includes(opcija) || process.argv.length > 3) {
    throw new Error('Upotreba: node scripts/uvoz-srpski2.mjs [--dry-run|--apply|--sql]')
  }
  const sql = napraviSqlUvoza(opcija === '--apply')
  if (opcija === '--sql') process.stdout.write(sql)
  else {
    const tmp = `/tmp/uvoz-srpski2-${opcija === '--apply' ? 'apply' : 'dry'}.sql`
