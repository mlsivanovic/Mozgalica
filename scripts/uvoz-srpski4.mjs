...
import { SRPSKI4_BANKA } from '../src/data/srpski4Banka.ts'

export function napraviSqlUvoza(primeni = false) {
  const payload = JSON.stringify(SRPSKI4_BANKA).replaceAll("'", "''")
  return `begin;
create temporary table srpski4_uvoz on commit drop as
...
begin
  ...
  if exists (
    select 1 from srpski4_uvoz u left join public.topics t on t.slug = u."topicSlug"
    where t.id is null or t.subject <> 'srpski' or t.grade <> 4
  ) then
    raise exception 'Nedostaju oblasti srpskog za 4. razred.';
  end if;

  if exists (
    select 1 from srpski4_uvoz u
    join public.topics t on t.slug = u."topicSlug"
    join public.questions q on q.topic_id = t.id
      and public.fn_normalize_text(q.text) = public.fn_normalize_text(u.text)
    where q.gen_signature is distinct from u.gen_signature
  ) then
    raise exception 'Tekst pitanja već postoji u oblasti 4. razreda; ništa nije upisano.';
  end if;
  ...
  from srpski4_uvoz u join public.topics t on t.slug = u."topicSlug"
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!['--dry-run', '--apply', '--sql'].includes(opcija) || process.argv.length > 3) {
    throw new Error('Upotreba: node scripts/uvoz-srpski4.mjs [--dry-run|--apply|--sql]')
  }
  const sql = napraviSqlUvoza(opcija === '--apply')
  if (opcija === '--sql') process.stdout.write(sql)
  else {
    const tmp = `/tmp/uvoz-srpski4-${opcija === '--apply' ? 'apply' : 'dry'}.sql`
