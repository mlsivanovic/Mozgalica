...
import { MATEMATIKA3_BANKA } from '../src/data/matematika3Banka.ts'

export function napraviSqlUvoza(primeni = false) {
  const payload = JSON.stringify(MATEMATIKA3_BANKA).replaceAll("'", "''")
  return `begin;
create temporary table matematika3_uvoz on commit drop as
...
declare
  ...
  v_broj int;
begin
  select count(distinct q.owner_id) into v_broj from public.questions q;
  if v_broj <> 1 then
    raise exception 'Uvoz zahteva tačno jednog vlasnika postojeće banke; pronađeno: %', v_broj;
  end if;
  select distinct q.owner_id into strict v_owner from public.questions q;

  if exists (
    select 1 from matematika3_uvoz u left join public.topics t on t.slug = u."topicSlug"
    where t.id is null or t.subject <> 'matematika' or t.grade <> 3
  ) then
    raise exception 'Nedostaju matematičke oblasti za 3. razred.';
  end if;
  ...
  from matematika3_uvoz u join public.topics t on t.slug = u."topicSlug"
  on conflict (owner_id, gen_signature) where gen_signature is not null do nothing;

  if exists (
    select 1 from matematika3_uvoz u
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!['--dry-run', '--apply', '--sql'].includes(opcija) || process.argv.length > 3) {
    throw new Error('Upotreba: node scripts/uvoz-matematika3.mjs [--dry-run|--apply|--sql]')
  }
  const sql = napraviSqlUvoza(opcija === '--apply')
  if (opcija === '--sql') process.stdout.write(sql)
  else {
    const tmp = `/tmp/uvoz-matematika3-${opcija === '--apply' ? 'apply' : 'dry'}.sql`
